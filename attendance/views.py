...
def mark_attendance(request):
    ...
    data = request.data
    required_fields = ['employee_id', 'date', 'check_in', 'type', 'status']
    ...
    if att_type == 'wfh':
        wfh_check = check_wfh_eligibility(employee_id, att_date)
...
def check_out(request):
    ...
    data = request.data
    required_fields = ['employee_id', 'date', 'check_out']
    ...
    try:
        ...
        check_in = datetime.combine(att_date, record.check_in_time)
        check_out = datetime.combine(att_date, datetime.strptime(check_out_time, '%H:%M:%S').time())
        ...
        return Response({
            'success': False,
            'message': 'No attendance record found for check-out'
        }, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
        return Response({
            'success': False,
